...
type DataHealth = {status:'WAITING'|'OK'|'DEGRADED'|'UNAVAILABLE';requested:number;analyzed:number;failures:number;rate_limited:number;cache_hit:boolean;cache_age_seconds?:number|null};
...
export default function Home() {
  ...
  const [exitMode,setExitMode]=useState<'RATCHET'|'REVERSAL'>('RATCHET'), [trailWindow,setTrailWindow]=useState(24);
  ...
  return <main>
    ...
    {tab==='home'?<HomeView summary={summary} runs={runs} latestRun={latestRun} activeRun={activeRun} activeRuns={activeRuns} initialCash={initialCash} totalPnl={totalPnl} duration={duration} setDuration={setDuration} allocation={allocation} setAllocation={setAllocation} maxPositions={maxPositions} setMaxPositions={setMaxPositions} exitMode={exitMode} setExitMode={setExitMode} trailWindow={trailWindow} setTrailWindow={setTrailWindow} entryTimeframe={entryTimeframe} setEntryTimeframe={setEntryTimeframe} cost={cost} busy={busy} startRun={startRun} stopRun={stopRun} openRun={openRun} onLaunched={refresh}/>:selectedRunId?(selectedRun?<RunDetail run={selectedRun} back={()=>setSelectedRunId(null)}/>:<section className="run-detail"><button className="back-button" onClick={()=>setSelectedRunId(null)}>← All runs</button><Empty text={detailError||'Loading the recorded trace…'}/></section>):tab==='reports'?<ReportsView openRun={openRun}/>:<RunsView runs={runs} openRun={openRun}/>}
    <footer><span>{notice}</span><span>Private local research · Paper execution only</span></footer>
  </main>;
}

function HomeView({summary,runs,latestRun,activeRun,activeRuns,initialCash,totalPnl,duration,setDuration,allocation,setAllocation,maxPositions,setMaxPositions,exitMode,setExitMode,trailWindow,setTrailWindow,entryTimeframe,setEntryTimeframe,cost,busy,startRun,stopRun,openRun,onLaunched}:{summary:Summary|null;runs:MomentumRun[];latestRun?:MomentumRun;activeRun?:MomentumRun;activeRuns:MomentumRun[];initialCash:number;totalPnl:number;duration:number;setDuration:(value:number)=>void;allocation:number;setAllocation:(value:number)=>void;maxPositions:number;setMaxPositions:(value:number)=>void;exitMode:'RATCHET'|'REVERSAL';setExitMode:(value:'RATCHET'|'REVERSAL')=>void;trailWindow:number;setTrailWindow:(value:number)=>void;entryTimeframe:number;setEntryTimeframe:(value:number)=>void;cost:CostModel|null;busy:string;startRun:()=>void;stopRun:()=>void;openRun:(id:string)=>void;onLaunched:()=>void;}) {
  const recentFills=latestRun?.fills.slice(-6).reverse()||[];
  return <><section className="hero home-hero"><div><p className="eyebrow">NIFTY 100 momentum research</p><h1>Track the move.<br/><em>Study the turn.</em></h1><p className="hero-copy">A focused paper account that surveys continuously, enters on three rising prints only when the move is larger than what the round trip costs, then manages the position with a stop that only ever moves up.</p></div><section className="new-run-card" aria-label="Start a new run"><div className="card-heading"><div><p className="eyebrow">New run</p><h2>Set the guardrails</h2></div><span className="paper-chip">PAPER</span></div><label>Duration<select value={duration} onChange={event=>setDuration(Number(event.target.value))} disabled={!!activeRun}><option value={300}>5 minutes</option><option value={600}>10 minutes</option><option value={900}>15 minutes</option><option value={1800}>30 minutes</option><option value={3600}>60 minutes</option></select></label><div className="form-pair"><label>Per position<input type="number" min="5000" max="200000" step="5000" value={allocation} onChange={event=>setAllocation(Number(event.target.value))} disabled={!!activeRun}/></label><label>Max positions<select value={maxPositions} onChange={event=>setMaxPositions(Number(event.target.value))} disabled={!!activeRun}><option value={1}>1</option><option value={2}>2</option><option value={3}>3</option><option value={4}>4</option></select></label></div><div className="form-pair"><label>Exit rule<select value={exitMode} onChange={event=>setExitMode(event.target.value as 'RATCHET'|'REVERSAL')} disabled={!!activeRun}><option value="RATCHET">Ratcheting stop</option><option value="REVERSAL">Fixed reversal</option></select></label><label>Trail window<input type="number" min="4" max="120" step="2" value={trailWindow} onChange={event=>setTrailWindow(Number(event.target.value))} disabled={!!activeRun||exitMode!=='RATCHET'}/></label></div><div className="form-pair"><div className="fixed-rule"><span>NIFTY 50 momentum</span><b>Recorded as context only</b></div><label>Entry timeframe<select value={entryTimeframe} onChange={event=>setEntryTimeframe(Number(event.target.value))} disabled={!!activeRun}><option value={0}>5-second ticks</option><option value={60}>1-minute bars</option></select></label></div>{entryTimeframe>0&&<div className="cost-hint"><span>Resampled entry</span><b>{entryTimeframe/60}-minute bars</b><small>The three-bar check waits for {entryTimeframe/60*3} minutes of closed bars before it can fire, and seeds the stop from each bar&rsquo;s real intrabar low rather than a five-second sample.</small></div>}{cost&&<div className={`cost-hint ${cost.breakeven_pct>0.2?'tight':''}`}><span>Breakeven at this size</span><b>{cost.breakeven_pct.toFixed(3)}%</b><small>{money.format(cost.total_cost)} of fees and slippage per round trip. {cost.breakeven_pct>0.2?'Flat brokerage dominates here — a larger position lowers this sharply.':'Costs are amortised well at this size.'}</small></div>}<div className="rule-line"><span>Entry</span><b>3-BAR{entryTimeframe>0?` · ${entryTimeframe/60}M`:' · 5S'}</b><span>Relative volume</span><b>≥1.20×</b><span>NIFTY short-term</span><b>CONTEXT ONLY</b><span>Exit</span><b>{exitMode==='RATCHET'?'RATCHET':'−0.10% REVERSAL'}</b></div>{activeRun?<button className="stop-run" onClick={stopRun} disabled={!!busy}>{busy==='stop'?'Closing positions…':'Stop current run'}</button>:<button className="start-run" onClick={startRun} disabled={!!busy||allocation<=0}>{busy==='start'?'Starting…':'Start new run'}<span>→</span></button>}</section></section>
...
function BatchLauncher({duration,allocation,maxPositions,disabled,onLaunched}:{duration:number;allocation:number;maxPositions:number;disabled:boolean;onLaunched:()=>void}) {
  ...
  const launch=async()=>{
    ...
    try{
      const body={base:{duration_seconds:duration,allocation_per_position:allocation,max_positions:maxPositions,entry_mode:'THREE_BAR',exit_mode:'RATCHET',require_nifty_confirmation:false},
                  account_prefix:'fwd',
  ...
  return <section className="panel batch-panel"><div className="panel-head"><div><p className="eyebrow">Run several configurations at once</p><h2>Comparison batch</h2></div><span className="count">{picked.length} selected</span></div>
    <p className="trace-help">The 5-second and 1-minute arms keep separate paper accounts but share one cached NIFTY 100 survey, avoiding duplicate market-data requests while preserving a fair comparison.</p>
...
function DataHealthNotice({run}:{run:MomentumRun}) {
  ...
  const reason=health.rate_limited>0?`${health.rate_limited} requests were rate-limited by Upstox.`:health.failures>0?`${health.failures} stock requests failed.`:'The first shared survey has not completed yet.';
  return <section className={`data-health ${unavailable?'unavailable':waiting?'waiting':'degraded'}`}><div><p className="eyebrow">Market data health</p><h2>{waiting?'Waiting for the first survey':unavailable?'Survey unavailable':'Survey partially available'}</h2><p>{waiting?reason:`Analyzed ${health.analyzed} of ${health.requested} NIFTY 100 stocks. ${reason} P&L and no-trade results are not conclusive while data is incomplete.`}</p></div><span>{health.cache_hit?'SHARED SNAPSHOT':'FRESH SURVEY'}</span></section>;
}
...
function CostFloorPanel({run}:{run:MomentumRun}) {
  ...
  return <><DataHealthNotice run={run}/><section className="panel cost-panel"><div className="panel-head"><div><p className="eyebrow">Cost floor</p><h2>What a round trip must clear</h2></div><span className="count">{money.format(cost.notional)} per position · {cost.product}</span></div>
